...
class SupervisorMainFrame( view_core.MainFrame ):
        ...
        if event.data is None:
            ...
            if "iosresults" in message:
                ...
                if self.InfoGrid.GetNumberRows() < len(self.ioports):
                    self.InfoGrid.AppendRows( len(self.ioports) - self.InfoGrid.GetNumberRows())
                ...
                for iop in self.ioports:
                    ...
                    self.InfoGrid.SetCellValue( self.lnumber, 1, valeur.decode("latin-1") )
                    self.lnumber+= 1
            #if "fuzz_status=" in message:
            if "archi=" in message:
                self.displayConsole("[Archi] %s: %s" % (source, message.split("=")[-1]))
                self.displayTree(self.model)
            else:
    ...
    def paintCutVM1( self ):
        self.dc.DrawCircle(self.rectVM1Pong.x + self.rectVM1Pong.width/2,
                           self.rectVM1Pong.y + self.rectVM1Pong.height + (self.rectHypPong.y - (self.rectVM1Pong.y + self.rectVM1Pong.height))/2,
                           8)
    ...
    def drawTreeItemMenu( self, event ):
        ...
        list_commands = eval(self.model.sendRemote(node, ("help|")))[0].split('#')
        self.menuPopup = []
        self.nodePopup = node
    ...
    def displayConsole( self, msg ):
        ...
        self.richTextConsole.ShowPosition(self.richTextConsole.GetLastPosition())
    ...
    def displayTree( self, model ):
        ...
        for ho in vo_slaves:
            ...
            for agent_name, agent_host, agent_port in ho_slaves:
                ...
                node_agent = self.archiTree.AppendItem(node_ho, agent_name, 0, 0, data)
            ...
            if type(item) == types.TupleType:
                if line_number == 0:
                    ...
                    for title_value in title_tuple:
                        self.InfoGrid.SetColLabelValue(title_num_col, title_tuple[title_num_col])
                        title_num_col+= 1
                    self.InfoGrid.AppendRows( len(result) )
                    self.InfoGrid.AppendCols( len(result[0]) )
                for value in item:
                    self.InfoGrid.SetCellValue( line_number, column_number, value.decode("latin-1") )
                    if "FOUND" in value:
                        for col in range(0, self.InfoGrid.GetNumberCols()):
                            self.InfoGrid.SetCellBackgroundColour( line_number, col, wx.RED )
                    else:
                        self.InfoGrid.SetCellBackgroundColour( line_number, column_number, wx.GREEN )
                    column_number+= 1
            elif type(item) == types.StringType:
                self.displayConsole( "Result: " + item.replace("|", "") )
            else:
                self.displayConsole( "Result: " + repr(item) + " " + str(type(item)))
            line_number+= 1
        self.InfoGrid.AutoSizeColumns()
