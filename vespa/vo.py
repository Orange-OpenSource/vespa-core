...
class VO(Node):
    ...
    def get_trans_bytes(self):
        return [ y['bytes']-x['bytes'] for x,y in zip(self.trans_bytes,self.trans_bytes[1:]) ][-40:]

    def get_recv_bytes(self):
        return [ y['bytes']-x['bytes'] for x,y in zip(self.recv_bytes,self.recv_bytes[1:]) ][-40:]
    ...
    def get_next_trans_bytes(self):
        if len(self.trans_bytes) > 2:
            item = self.trans_bytes[-1]['bytes'] - self.trans_bytes[-2]['bytes']
        else:
            item = 0
        return item
    ...
    def alert(self, msg):
        ...
        if source == "Agent_Bandwidth":
            if "recv_bytes" in message:
                ...
                #self.sendRemotef(self.model, "alert|%s>recv_bytes#%s#%s" % (self.name, tm, r))
                ...
                #self.sendRemotef(self.model, "alert|%s>trans_bytes#%s#%s" % (self.name, tm, r))
            else:
            ...
            if "ip_connections" in message:
                ...
                if len(self.ip_connections) > 0:
                    for sip in self.ip_connections:
                        sip['value'] = 0
                for ipobj in a:
                    if len(self.ip_connections) == 0 or ipobj['ip'] not in [ selfip['ip'] for selfip in self.ip_connections ]:
                ...
                if new_ddos and not self.under_ddos:
                    ...
                    if (time.time() - self.time_last_attack) > 30:
                        ...
                        #agent_controller = self.findNode("Agent_Controller_Pox")
                        agent_controller = self.findNode("Agent_Controller_Floodlight")
                        mac = self.sendRemote(agent_bp, 'get_mac|')
                        self.sendRemotef(agent_controller, "alert_ip|%s#%s" % (ipobj['value'], mac))
                        debug_info("DDos detected for %s, forwarding %s" % (ipobj['value'], mac))
                    ...
                    if (time.time() - self.time_last_attack) > 120:
                        ...
                        mac = self.sendRemote(agent_bp, 'get_mac|')
                        self.sendRemotef(agent_controller, "alert_ip|%s#%s" % (ipobj['value'], mac))
                        debug_info("DDos stopped for %s, normal traffic for %s" % (ipobj['value'], mac))
            else:
                self.sendRemotef(self.model, "alert|%s>Unexpected alert: %s" % (self.name, message))
