...
class Agent_Bandwidth(Node):
    ...
    def launch(self):
        ...
        while not self.quitting:
            ...
            #self.sendRemote(self.master, "alert|%s>recv_bytes#%s#%s" % (self.name, tm, r), needack=False)
            self.sendAlert("recv_bytes#%s#%s" % (tm, r))

            t = infos[self.iface]['trans_bytes']
            #self.sendRemote(self.master, "alert|%s>trans_bytes#%s#%s" % (self.name, tm, t), needack=False)
